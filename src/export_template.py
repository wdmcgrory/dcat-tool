...
def export_template(include_instructions, ci_objs):
    eg = easy_workbook.ExcelGenerator()
    if include_instructions:
        eg.add_markdown_sheet("Instructions", open(INSTRUCTIONS).read())
    eg.add_columns_sheet("Inventory", ci_objs)
    sys.stdout.buffer.write(eg.export())
...
if __name__=="__main__":
    ...
    parser.add_argument("--export_template", help="return the Excel template file to make for a collection schema to standard output", action='store_true')
    ...
    export_template(not args.noinstructions, ci_objs)
